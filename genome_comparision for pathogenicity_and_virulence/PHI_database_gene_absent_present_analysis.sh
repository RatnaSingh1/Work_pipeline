...
#from roary pan genome analysis gene list gene_presence_absence.csv obtained

#   *.fna genome run again the PHI database (phi-base_current.fas) to find the putative pathogenic genes with a cutoff 30%identity and 50% query cover output_parameters_50.txt
...
blastp -query PROKKA_10022024.faa -subject phi-base_current.fas  -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen' | awk '$3 >= 70 && ($4/$14)*100 >= 50' >> output_parameters_70.txt
